...
import { 
  ...
  Trash2, Edit2, Save, Folder, Cpu, Globe, Zap, UploadCloud, CheckCircle, Search, ArrowLeft, Download, Waves
} from 'lucide-react';
...
export default function App() {
  ...
  // --- Estados de UI ---
  const [currentView, setCurrentView] = useState<'chat' | 'admin' | 'settings' | 'projects' | 'agents' | 'sanipes'>('chat');
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-[#1e1e2e] flex items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-[#181825] rounded-2xl shadow-2xl border border-[#313244] p-8">
          <div className="flex justify-center mb-6"><div className="w-16 h-16 rounded-2xl bg-[#00509e] flex items-center justify-center"><Waves size={32} className="text-white" /></div></div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="text" value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} className="w-full bg-[#313244] border border-[#45475a] text-white rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#00509e]" placeholder="admin o usuario1" required />
            </div>
            <div>
              <label className="block text-sm text-gray-300 mb-1">Contraseña</label>
              <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} className="w-full bg-[#313244] border border-[#45475a] text-white rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#00509e]" placeholder="••••••••" required />
            </div>
            {loginError && <div className="text-red-400 text-sm text-center">{loginError}</div>}
            <button type="submit" className="w-full bg-[#003b73] hover:bg-[#00509e] text-white font-medium py-2.5 rounded-lg mt-4">Ingresar</button>
          </form>
        </div>
      </div>
    );
  ...
  // ==========================================
  return (
    <div className="flex h-screen bg-[#1e1e2e] text-gray-100 font-sans overflow-hidden">
      {/* Sidebar */}
      <div className={`${isSidebarOpen ? 'w-64' : 'w-0'} transition-all duration-300 bg-[#181825] flex flex-col border-r border-[#313244] shrink-0`}>
        <div className="p-4 flex items-center gap-3 border-b border-[#313244]">
          <div className="w-8 h-8 rounded-lg bg-[#00509e] flex items-center justify-center"><Waves size={20} className="text-white" /></div>
          <span className="font-semibold text-lg">COINREFRI AI</span>
        </div>
        <div className="p-3">
          <button onClick={openNewChatModal} className="w-full flex items-center gap-2 bg-[#003b73] hover:bg-[#00509e] text-white px-4 py-2.5 rounded-lg text-sm font-medium">
            <Plus size={18} /> Nuevo Chat
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          <div>
            <div className="text-xs font-semibold text-[#a6adc8] uppercase mb-2 px-2">Espacio de Trabajo</div>
            <div className="space-y-1">
              <button onClick={() => setCurrentView('projects')} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${currentView === 'projects' ? 'bg-[#313244] text-emerald-400' : 'hover:bg-[#313244] text-gray-400'}`}><Folder size={16} /> Proyectos</button>
              <button onClick={() => {setCurrentView('agents'); setActiveAgentId(null);}} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${currentView === 'agents' ? 'bg-[#313244] text-purple-400' : 'hover:bg-[#313244] text-gray-400'}`}><Cpu size={16} /> Agentes Generados</button>
              <button onClick={() => setCurrentView('sanipes')} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${currentView === 'sanipes' ? 'bg-[#313244] text-blue-400' : 'hover:bg-[#313244] text-gray-400'}`}><Globe size={16} /> SANIPES</button>
            </div>
          </div>
          <div>
            <div className="text-xs font-semibold text-[#a6adc8] uppercase mb-2 px-2">Chats Recientes</div>
            <div className="space-y-1">
              {userChats.map(chat => (
                <div key={chat.id} className={`group flex items-center justify-between px-3 py-2 rounded-lg text-sm ${currentChatId === chat.id && currentView === 'chat' ? 'bg-[#313244] text-[#4da6ff]' : 'hover:bg-[#313244] text-gray-400'}`}>
        ...
        <div className="p-3 border-t border-[#313244] space-y-1">
          {currentUser.role === 'admin' && (
            <>
              <button onClick={() => setCurrentView('admin')} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${currentView === 'admin' ? 'bg-[#313244] text-emerald-400' : 'hover:bg-[#313244] text-gray-400'}`}><Shield size={18} /> Panel Admin</button>
              <button onClick={() => setCurrentView('settings')} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${currentView === 'settings' ? 'bg-[#313244] text-blue-400' : 'hover:bg-[#313244] text-gray-400'}`}><Settings size={18} /> Configuración</button>
            </>
          )}
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-500/10 text-gray-400 hover:text-red-400 text-sm mt-2"><LogOut size={18} /> Cerrar Sesión</button>
        </div>
      </div>

      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 bg-[#1e1e2e]">
        <header className="h-14 flex items-center px-4 border-b border-[#313244] bg-[#1e1e2e]/80 shrink-0 gap-3">
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-[#313244] rounded-lg text-gray-400"><Menu size={20} /></button>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-6xl mx-auto space-y-8">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Chart 1: Tokens */}
                <div className="bg-[#181825] border border-[#313244] p-5 rounded-xl">
                ...
                {/* Chart 3: Tool Usage */}
                <div className="bg-[#181825] border border-[#313244] p-5 rounded-xl md:col-span-2 lg:col-span-1">
              ...
              <h2 className="text-2xl font-bold text-white pt-6 border-t border-[#313244]">Gestión de Usuarios</h2>
              <div className="grid md:grid-cols-3 gap-6">
                <form onSubmit={handleCreateUser} className="bg-[#181825] border border-[#313244] p-5 rounded-xl h-fit space-y-4">
                  <h3 className="font-medium text-white flex items-center gap-2"><UserPlus size={18} className="text-emerald-400"/> Nuevo Usuario</h3>
                  <input type="text" value={newUsername} onChange={e=>setNewUsername(e.target.value)} placeholder="Username" className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" required />
                  <select value={newUserRole} onChange={e=>setNewUserRole(e.target.value as Role)} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]">
                    <option value="user">Usuario</option><option value="admin">Admin</option>
                  </select>
                  <button type="submit" className="w-full bg-emerald-600 text-white py-2 rounded-lg">Crear</button>
                </form>
                <div className="md:col-span-2 bg-[#181825] border border-[#313244] rounded-xl overflow-hidden">
                  <table className="w-full text-left text-sm text-gray-400">
                    <thead className="bg-[#313244]/50 text-xs uppercase"><tr><th className="px-4 py-3">Usuario</th><th className="px-4 py-3">Rol</th><th className="px-4 py-3 text-right">Acción</th></tr></thead>
                    <tbody>
                      {usersDb.map(u => (
                        <tr key={u.id} className="border-b border-[#313244]">
        ...
        {currentView === 'settings' && currentUser.role === 'admin' && (
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              <form onSubmit={handleSaveSettings} className="bg-[#181825] border border-[#313244] p-6 rounded-xl space-y-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Modelo LLM</label>
                  <select value={systemSettings.model} onChange={e=>setSystemSettings({...systemSettings, model: e.target.value})} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]">
                ...
                <div>
                  ...
                  <textarea value={systemSettings.systemPrompt} onChange={e=>setSystemSettings({...systemSettings, systemPrompt: e.target.value})} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" rows={4} />
                </div>
                <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Save size={16}/> Guardar</button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-5xl mx-auto space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowProjectForm(!showProjectForm)} className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2"><Plus size={16}/> Crear Proyecto</button>
              </div>
              {showProjectForm && (
                <form onSubmit={handleCreateProject} className="bg-[#181825] border border-[#313244] p-5 rounded-xl space-y-3">
                  <input type="text" value={newProject.name} onChange={e=>setNewProject({...newProject, name: e.target.value})} placeholder="Nombre del Proyecto" className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" required />
                  <textarea value={newProject.description} onChange={e=>setNewProject({...newProject, description: e.target.value})} placeholder="Descripción" className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" rows={2} />
                  <button type="submit" className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm">Guardar</button>
                </form>
              )}
              {projects.filter(p => p.userId === currentUser.id).length === 0 && !showProjectForm && (
                <div className="text-center py-12 bg-[#181825] border border-dashed border-[#45475a] rounded-xl">
                  ...
                  <button onClick={() => setShowProjectForm(true)} className="text-emerald-400 hover:text-emerald-300 font-medium">Crear mi primer proyecto</button>
                </div>
              )}
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {projects.filter(p => p.userId === currentUser.id).map(p => (
                  <div key={p.id} className="bg-[#181825] border border-[#313244] p-5 rounded-xl flex flex-col relative group">
                    ...
                    <h3 className="font-medium text-white flex items-center gap-2 mb-2 pr-6"><Folder className="text-emerald-400" size={18}/> {p.name}</h3>
                    <p className="text-sm text-gray-400 mb-4 flex-1">{p.description}</p>
                    <div className="bg-[#313244] rounded-lg p-3 mb-4 text-center relative border border-dashed border-[#45475a] hover:border-emerald-500">
                    ...
                    <button onClick={() => handleNewChat(currentUser.id, null, p.id)} className="w-full py-2 bg-[#313244] hover:bg-[#45475a] text-white rounded-lg text-sm">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-5xl mx-auto space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowAgentForm(!showAgentForm)} className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2"><Plus size={16}/> Crear Agente</button>
              </div>
              {showAgentForm && (
                <form onSubmit={handleCreateAgent} className="bg-[#181825] border border-[#313244] p-5 rounded-xl space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" value={newAgent.name} onChange={e=>setNewAgent({...newAgent, name: e.target.value})} placeholder="Nombre del Agente" className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" required />
                    <select value={newAgent.projectId} onChange={e=>setNewAgent({...newAgent, projectId: e.target.value})} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]">
                  ...
                  <textarea value={newAgent.instructions} onChange={e=>setNewAgent({...newAgent, instructions: e.target.value})} placeholder="Instrucciones del sistema (Prompt)" className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a]" rows={2} required />
                  <button type="submit" className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm">Guardar Agente</button>
                </form>
              )}
              {agents.filter(a => a.userId === currentUser.id).length === 0 && !showAgentForm && (
                <div className="text-center py-12 bg-[#181825] border border-dashed border-[#45475a] rounded-xl">
                  ...
                  <button onClick={() => setShowAgentForm(true)} className="text-purple-400 hover:text-purple-300 font-medium">Crear mi primer agente</button>
                </div>
              )}
              <div className="grid md:grid-cols-2 gap-4">
                {agents.filter(a => a.userId === currentUser.id).map(a => (
                  <div key={a.id} className="bg-[#181825] border border-[#313244] p-5 rounded-xl flex flex-col">
                    <h3 className="font-medium text-white flex items-center gap-2 mb-2"><Cpu className="text-purple-400" size={18}/> {a.name}</h3>
                    ...
                    }} className="w-full py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm">
        ...
        {currentView === 'sanipes' && (
          <div className="flex-1 overflow-y-auto p-6 bg-[#1e1e2e]">
            <div className="max-w-4xl mx-auto space-y-6">
              <div className="bg-[#181825] border border-[#313244] p-6 rounded-xl shadow-lg">
                <div className="flex items-center gap-3 mb-6 border-b border-[#313244] pb-4">
                ...
                <div className="bg-[#1e1e2e] border border-[#313244] rounded-lg p-5 mb-6">
                  ...
                  <button onClick={handleSanipesSearch} disabled={isSanipesLoading} className="w-full bg-[#003b73] hover:bg-[#00509e] disabled:bg-[#002855] text-white py-2.5 rounded-md font-medium flex items-center justify-center gap-2 transition-colors">
                ...
                {sanipesResult && (
                  <div className="bg-[#1e1e2e] border border-[#313244] rounded-lg overflow-hidden shadow-lg animate-in fade-in slide-in-from-bottom-4">
                    <div className="bg-[#003b73] px-5 py-3 border-b border-[#002855] flex items-center gap-2">
                    ...
                    <div className="p-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div>
                          ...
                          <span className="inline-block px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded-full text-sm font-bold border border-emerald-500/30">{sanipesResult.estado}</span>
                        </div>
                      </div>
                      
                      <div className="mt-8 pt-4 border-t border-[#313244] flex justify-between items-center">
                        <span className="text-gray-300 text-sm font-medium">Ver Listado de Protocolos:</span>
                        <button onClick={() => setShowProtocolosModal(true)} className="text-[#4da6ff] hover:text-white hover:underline font-medium flex items-center gap-1 text-sm transition-colors">
                          Descargar
                        </button>
                      </div>
                      <div className="mt-6">
                        <button onClick={handleOpenSanipesChat} className="w-full bg-[#003b73] hover:bg-[#00509e] text-white py-2.5 rounded-md text-sm font-medium flex items-center justify-center gap-2 transition-colors"><MessageSquare size={16}/> Chat de Consulta</button>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {showSanipesChat && (
                <div className="bg-[#181825] border border-[#313244] rounded-xl flex flex-col h-[500px] shadow-xl animate-in slide-in-from-bottom-4">
                  <div className="bg-[#1e1e2e] p-4 border-b border-[#313244] flex justify-between items-center rounded-t-xl">
                    <div className="flex items-center gap-3">
                      <Waves size={20} className="text-[#4da6ff]"/>
                      <div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {sanipesChatMessages.length === 0 ? (
                      <div className="text-center text-gray-500 mt-10">
                        <FileText size={40} className="mx-auto mb-3 opacity-50 text-[#4da6ff]"/>
                      ...
                      sanipesChatMessages.map(msg => (
                        <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                          {msg.role === 'assistant' && <div className="w-8 h-8 rounded-full bg-[#00509e] flex items-center justify-center flex-shrink-0"><Waves size={16} className="text-white" /></div>}
                          <div className={`max-w-[80%] rounded-2xl p-3 text-sm ${msg.role === 'user' ? 'bg-[#004b87] text-white rounded-tr-sm' : 'bg-[#313244] text-gray-200 rounded-tl-sm'}`}>
                    ...
                    {isSanipesChatTyping && (
                      <div className="flex gap-3 justify-start">
                        <div className="w-8 h-8 rounded-full bg-[#00509e] flex items-center justify-center flex-shrink-0"><Waves size={16} className="text-white" /></div>
                        <div className="bg-[#313244] rounded-2xl rounded-tl-sm p-4 flex items-center gap-1">
                  ...
                  <div className="p-4 border-t border-[#313244] bg-[#1e1e2e] rounded-b-xl">
                    <form onSubmit={handleSanipesChatSubmit} className="flex gap-2">
                      <input 
                        ...
                        className="flex-1 bg-[#313244] text-white rounded-lg px-4 py-2 border border-[#45475a] focus:border-[#00509e] outline-none text-sm"
                      />
                      <button type="submit" disabled={!sanipesChatInput.trim() || isSanipesChatTyping} className="bg-[#003b73] hover:bg-[#00509e] disabled:bg-[#002855] disabled:text-gray-400 text-white p-2 rounded-lg transition-colors">
                        <Send size={18} />
                      </button>
                    </form>
                  </div>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-sm w-full max-w-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4 border-t border-gray-200 bg-gray-100 flex justify-end">
                      <button onClick={() => setShowProtocolosModal(false)} className="bg-[#444] hover:bg-[#333] text-white px-6 py-2 rounded-sm text-sm">Salir</button>
                    </div>
                  </div>
                </div>
              )}
        ...
        {(currentView === 'chat' || (currentView === 'agents' && activeAgentId)) && !currentChat && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
            <div className="w-24 h-24 bg-[#313244] rounded-3xl flex items-center justify-center mb-6 shadow-lg border border-[#45475a]">
              <Waves size={48} className="text-[#4da6ff]" />
            </div>
            ...
            <button onClick={openNewChatModal} className="bg-[#003b73] hover:bg-[#00509e] text-white px-6 py-3 rounded-xl font-medium flex items-center gap-2 transition-all hover:scale-105 shadow-lg shadow-[#00509e]/20">
        ...
        {(currentView === 'chat' || (currentView === 'agents' && activeAgentId)) && currentChat && (
          <div className="flex-1 flex flex-col min-h-0">
            {currentView === 'agents' && activeAgentId && (
              <div className="bg-[#313244] px-4 py-2 flex items-center gap-3 border-b border-[#45475a]">
                ...
                <span className="text-purple-400 font-medium text-sm">| {agents.find(a=>a.id===activeAgentId)?.name}</span>
              </div>
            )}
            {currentChat.attachedDocument && (
              <div className="bg-[#1e1e2e] px-6 py-3 flex items-center justify-between border-b border-[#313244]">
                <div className="flex items-center gap-3">
                ...
                <button onClick={() => handleDownloadSanipesPDF(currentChat.attachedDocument?.data)} className="text-xs bg-[#313244] hover:bg-[#45475a] text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors border border-[#45475a]">
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                {currentChat.messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'assistant' && (
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-1 ${currentAgent ? 'bg-[#00509e]' : currentProject ? 'bg-emerald-500' : 'bg-[#00509e]'}`}>
                    ...
                    <div className={`flex flex-col gap-2 max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`px-4 py-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-[#004b87] text-white rounded-tr-sm' : 'bg-[#313244] text-gray-200 rounded-tl-sm border border-[#45475a]'}`}>
                        {msg.content}
                      </div>
                    </div>
                  </div>
                ...
                {isTyping && (
                  <div className="flex gap-4 justify-start">
                    <div className="w-8 h-8 rounded-lg bg-[#00509e] flex items-center justify-center shrink-0 mt-1"><Loader2 size={18} className="text-white animate-spin"/></div>
                  </div>
                )}
                <div ref={messagesEndRef} />
              </div>
            </div>
            <div className="p-4 bg-[#1e1e2e] shrink-0">
              <div className="max-w-3xl mx-auto">
                {currentAgent && currentAgent.quickResponses.length > 0 && (
                  <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
                    {currentAgent.quickResponses.map((qr, idx) => (
                      <button key={idx} onClick={() => handleSend(qr)} className="whitespace-nowrap px-4 py-1.5 bg-[#313244] hover:bg-purple-500/20 hover:text-purple-400 border border-[#45475a] hover:border-purple-500/50 rounded-full text-xs text-gray-300 transition-all">
                        {qr}
                      </button>
                    ))}
                  </div>
                ...
                {attachments.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {attachments.map((file, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-[#313244] px-3 py-1.5 rounded-lg text-xs border border-[#45475a]">
                        <Paperclip size={14} className="text-[#4da6ff]" />
                ...
                <div className="relative bg-[#313244] rounded-xl border border-[#45475a] flex items-end p-2 focus-within:border-[#00509e] transition-colors">
                  <label className="p-2 text-gray-400 hover:text-[#4da6ff] cursor-pointer shrink-0 transition-colors">
                  ...
                  <textarea 
                    ...
                    rows={1}
                  />
                  <button onClick={()=>handleSend()} disabled={!input.trim() && attachments.length === 0} className="p-2 bg-[#003b73] hover:bg-[#00509e] disabled:bg-[#45475a] disabled:text-gray-400 text-white rounded-lg shrink-0 transition-colors mb-0.5 mr-0.5"><Send size={18}/></button>
                </div>
              </div>
            </div>
          </div>
      ...
      {showNewChatModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#181825] p-6 rounded-xl border border-[#313244] max-w-md w-full shadow-2xl">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => { setSelectedNewChatType('general'); setSelectedNewChatId(''); }} className={`py-2 rounded-lg text-sm font-medium border transition-colors ${selectedNewChatType === 'general' ? 'bg-[#003b73] border-[#00509e] text-white' : 'bg-[#313244] border-[#45475a] text-gray-400 hover:bg-[#45475a]'}`}>General</button>
                <button onClick={() => { setSelectedNewChatType('project'); setSelectedNewChatId(''); }} className={`py-2 rounded-lg text-sm font-medium border transition-colors ${selectedNewChatType === 'project' ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-[#313244] border-[#45475a] text-gray-400 hover:bg-[#45475a]'}`}>Proyecto</button>
                <button onClick={() => { setSelectedNewChatType('agent'); setSelectedNewChatId(''); }} className={`py-2 rounded-lg text-sm font-medium border transition-colors ${selectedNewChatType === 'agent' ? 'bg-purple-600 border-purple-500 text-white' : 'bg-[#313244] border-[#45475a] text-gray-400 hover:bg-[#45475a]'}`}>Agente</button>
              </div>

              {selectedNewChatType === 'project' && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                  <label className="block text-sm text-gray-400 mb-1">Selecciona un Proyecto</label>
                  <select value={selectedNewChatId} onChange={e => setSelectedNewChatId(e.target.value)} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a] focus:outline-none focus:border-emerald-500">
              ...
              {selectedNewChatType === 'agent' && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <select value={selectedNewChatId} onChange={e => setSelectedNewChatId(e.target.value)} className="w-full bg-[#313244] text-white rounded-lg px-3 py-2 border border-[#45475a] focus:outline-none focus:border-purple-500">
              ...
              <button 
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2.5 rounded-lg mt-4 font-medium transition-colors"
              >
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#181825] p-6 rounded-xl border border-[#313244] max-w-sm w-full shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                className={`px-4 py-2 text-white rounded-lg transition-colors ${modal.type === 'confirm' ? 'bg-red-600 hover:bg-red-500' : 'bg-[#003b73] hover:bg-[#00509e]'}`}
              >
